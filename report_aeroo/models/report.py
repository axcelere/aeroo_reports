...
class Parser(models.AbstractModel):
    ...
    report_type = fields.Selection(selection_add=[('aeroo', _('Aeroo Reports'))], ondelete={'aeroo': 'set default'})
            ...
            for mod_path in module.ad_paths:
                if os.path.lexists(mod_path+os.path.sep+path.split(os.path.sep)[0]):
                    ...
                    if file_ext.lower() == '.py':
                        py_mod = importlib.util.spec_from_file_location(mod_name, filepath)

                    elif file_ext.lower() == '.pyc':
                        py_mod = imp.spec_from_loader(mod_name, filepath)
